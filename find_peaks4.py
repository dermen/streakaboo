...
def pk_pos3( img_, make_sparse=True, nsigs=7, sig_G=None, thresh=1, sz=4, min_snr=2.,
    ...
    if filt2:
        ...
        par = {'area_cut':1.2, 
            ...
            'radius_dev_cut':0.0, 
            'shape_cut':1.}
        Y,X = map( np.array, zip(*pos) )
        if median_sub:
            m1 = img_.mean()
            m2 = median_img.mean()
            S = streak_peak.SubImages( img_ - median_img*(m1/m2), Y, X, sz=sz, mask=mask, cent=cent)  
        else:
            S = streak_peak.SubImages( img_, Y, X, sz=sz, mask=mask, cent=cent)  
        for ii,s in enumerate(S.sub_imgs):
            s.integrate_pred_streak( sig_G=sig_G2, dist_cut=dist_cut, **par)
            S.sub_imgs[ii] = s
            if not s.has_streak:
                continue
            if s.lab_dist > dist_cut:
                continue
            
            new_pos.append( [s.streak_COM[0], s.streak_COM[1]] )
            new_intens.append( s.counts )
            subs.append( s.img)
        pos = new_pos
        intens = new_intens
    ...
    if filt2 and not ret_subs:
        return pos, intens
    if ret_subs and filt2:
        return pos, intens, subs
    if filt and not ret_subs:
        return pos, intens
    if ret_subs and filt:
        return pos, intens, subs
    else:
        return pos, intens
...
def make_cxi_file4( img_gen, outname, Hsh, 
    mask=None,
    dtype=np.float32, 
    verbose=True,
    compression=None,
    ...
    **kwargs):
    ...
    with h5py.File( outname, "w") as out:
        ...
        count = 0
        hit_fnames = []
        for i_h,(fname,h) in enumerate(img_gen): 
